...
三线汇合，导出"五秒赌注"作为元认知支架，并将问题从控制论的能观框架重新归属为元认知问题。结论是**条件性的**：若经验前提为真，则方法论提升效率；前提之真伪尚未经数据检验。
...
| 路径 | 内容 |
|---|---|
| `public/` | 框架正文与静态站点源。权威 ground truth：`SPEF_paper_v3.md`（完整论证与全部形式化主张）、`SPEF_diagram.html`（每个章节到三线证明逻辑角色的映射）。另含 `SPEF_handbook.md`（指导手册）、`SPEF_production_card.md`、`card/`（视觉卡片）。 |
| `site/` | Astro 静态站点，单一来源读取 `public/`。 |
...
- 第一次读，或想看完整论证：`public/SPEF_paper_v3.md`
- 日常执行的操作手册（含四阶段、五秒赌注、押注判据、验证协议、终止条件）：`public/SPEF_handbook.md`
- 论证结构地图（三线 A/B/C 与各章节的对应）：`public/SPEF_diagram.html`
- 生产用速查卡：`public/SPEF_production_card.md` 与 `public/card/`
...
- `private/`、`CLAUDE.md`、`LAYERING.md` 等内部未发布文件——**保留所有权利**
