...
| 路径 | 内容 |
|---|---|
| `public/` | 框架正文与静态站点源。权威 ground truth：`SPEF_paper_v6.md`（完整论证与全部形式化主张）、`SPEF_diagram.html`（每个章节到三线证明逻辑角色的映射）。另含 `SPEF_handbook.md`（指导手册）、`SPEF_production_card.md`、`card/`（视觉卡片）。 |
...
- 第一次读，或想看完整论证：`public/SPEF_paper_v6.md`
- 日常执行的操作手册（含四阶段、瞬时预判押注、押注判据、验证协议、终止条件）：`public/SPEF_handbook.md`
